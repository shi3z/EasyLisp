...
def add_globals(env):
    ...
    env.update({
        ...
        'cadr': lambda x: x[1] if len(x) > 1 else None, 'caar': lambda x: x[0][0], 
    ...
    def quasiquote(self, x, env):
        ...
        for elem in x:
            ...
            if isinstance(elem, list):
                if len(elem) > 0 and elem[0] == Symbol('unquote'):
...
def quasiquote(x, env):
    ...
    return [quasiquote(elem, env) if isinstance(elem, list) else
            eval(Sym(str(elem)[1:]), env) if isinstance(elem, Symbol) and str(elem).startswith(',')
            else elem
            for elem in x]

def unquote_splicing(x, env):
    result = eval(x, env)
    if not isinstance(result, list):
        raise TypeError("unquote-splicing expects a list")
    return result
